...
final class CoreLocationProximityEngine: ProximityEngine {
    ...
    func start() {
        ...
        monitorTask = Task { [weak self] in await self?.syncMonitor(enabled: backgroundEnabled) }
    ...
    private func syncMonitor(enabled: Bool) async {
        ...
        for identifier in await monitor.identifiers where !currentSlugs.contains(identifier) {
            await monitor.remove(identifier)
        }
        ...
        for site in content.allSites() {
            ...
            await monitor.add(
                CLMonitor.CircularGeographicCondition(
                    ...
                    radius: max(site.triggerRadiusMeters, AppConfig.monitorWakeRadiusMeters)
                ),
