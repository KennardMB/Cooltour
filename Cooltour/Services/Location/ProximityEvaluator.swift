...
struct ProximityEvaluator {
  ...
  func accepts(horizontalAccuracyMeters: Double) -> Bool {
    horizontalAccuracyMeters > 0
      && horizontalAccuracyMeters <= maxAccuracyMeters
  }
  ...
  mutating func evaluate(
    ...
    guard accepts(horizontalAccuracyMeters: horizontalAccuracyMeters) else {
      return []
    }
    ...
    for candidate in candidates.sorted(by: {
      ...
      } else if candidate.distanceMeters > candidate.triggerRadiusMeters
        * reArmMultiplier
      {
