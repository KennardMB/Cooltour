...
final class LocalContentStore: ContentStore {
    ...
    static func inMemory(packName: String = AppConfig.contentPackName) -> LocalContentStore {
        let container = try! ModelContainer(
            for: Site.self, Story.self,
    ...
    func seedIfNeeded() throws {
        ...
        guard seeded != pack.contentPackVersion || existingCount == 0 else { return }
        ...
        try context.save()
        UserDefaults.standard.set(pack.contentPackVersion, forKey: Self.seededVersionKey)
    }
