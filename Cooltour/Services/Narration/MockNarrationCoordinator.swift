...
/// Does nothing on its own — the real rules live in `ConsentNarrationCoordinator` (Slice 11b).
/// It exists so `AppEnvironment`'s default init has a coordinator for previews and tests, and so
/// Slices 13 and 14 can develop against a stable interface while the real implementation is still
/// being written. `simulatePrompt` drives the observable state by hand so a `#Preview` can show
/// every screen state without proximity or audio.
@Observable
