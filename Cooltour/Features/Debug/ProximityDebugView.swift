...
struct ProximityDebugView: View {
    ...
    private var engine: CoreLocationProximityEngine? { env.proximity as? CoreLocationProximityEngine }
    ...
    private var engineSection: some View {
        Section("Engine") {
            ...
            LabeledContent("Permission", value: env.proximity.authorizationStatus.displayName)
    ...
    private var wakeText: String {
        guard let wake = engine?.lastWake else { return backgroundTriggering ? "None yet" : "—" }
        return "\(wake.siteSlug) · \(wake.date.formatted(date: .omitted, time: .shortened))"
    }
    ...
    private var fixSection: some View {
        Section("Last fix") {
            if let fix = env.proximity.lastFix {
                LabeledContent("Coordinate", value: String(format: "%.5f, %.5f", fix.latitude, fix.longitude))
                LabeledContent("Accuracy") {
                    Label("±\(Int(fix.horizontalAccuracyMeters)) m", systemImage: fix.isTrusted ? "checkmark.circle.fill" : "xmark.octagon.fill")
                        .foregroundStyle(fix.isTrusted ? .green : .red)
                        .labelStyle(.titleAndIcon)
                }
                .accessibilityLabel(fix.isTrusted
                ...
                LabeledContent("At", value: fix.date.formatted(date: .omitted, time: .standard))
                if !fix.isTrusted {
                    Text("Worse than \(Int(AppConfig.maxLocationAccuracyMeters)) m — staying silent.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            } else {
                Text(env.proximity.isListening ? "Waiting for a fix…" : "Not listening.")
                    .foregroundStyle(.secondary)
            }
        }
    }
        ...
        Section("Triggers (\(env.proximity.recentEvents.count))") {
            ...
            ForEach(env.proximity.recentEvents) { event in
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("\(event.date.formatted(date: .abbreviated, time: .standard)) · \(Int(event.distanceMeters)) m · ±\(Int(event.horizontalAccuracyMeters)) m")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .monospacedDigit()
                }
            ...
            ForEach(env.proximity.nearbySites) { nearby in
                HStack {
                    ...
                    Text("\(Int(nearby.distanceMeters)) m")
                        .monospacedDigit()
                        .foregroundStyle(nearby.isInsideRadius ? Color.green : Color.primary)
                    Image(systemName: nearby.isArmed ? "bell.fill" : "bell.slash.fill")
                        .foregroundStyle(nearby.isArmed ? Color.secondary : Color.orange)
                        .accessibilityLabel(nearby.isArmed ? "Armed" : "Already played, waiting to re-arm")
                }
            }
...
#Preview {
    ...
    .task {
        ...
        guard let engine = env.proximity as? MockProximityEngine,
              let site = env.content.allSites().first else { return }
        engine.nearbySites = env.content.allSites().enumerated().map { index, site in
            NearbySite(
                id: site.slug,
                name: site.name,
        ...
        engine.lastFix = ProximityFix(
            ...
            horizontalAccuracyMeters: 9, date: .now, isTrusted: true
        )
        engine.start()
